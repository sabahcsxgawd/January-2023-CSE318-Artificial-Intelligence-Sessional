...
public:
    ...
    ll GREEDY_MAXCUT()
    {
        ...
        while (remainingCnt--)
        {
            int i = -1;
            maxW = LLONG_MIN;
            for(int j : remainingNodes) {
                if (max(get<1>(this->sigmas[j]), get<2>(this->sigmas[j])) >= maxW)
                {
                    maxW = max(get<1>(this->sigmas[j]), get<2>(this->sigmas[j]));
                    i = j;
                }
            }
            tempMap = this->g->getAdjListWeights(i);
            remainingNodes.erase(i);
